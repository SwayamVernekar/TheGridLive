...
async function fetchFromErgast(endpoint) {
  const url = `${ERGAST_API_URL}/${endpoint}.json?limit=100`;
  console.log('  [fetchFromErgast] Fetching:', url);
  
  try {
    const response = await fetch(url);
    console.log('  [fetchFromErgast] Status:', response.status);
    ...
    const data = await response.json();
    console.log('  [fetchFromErgast] Success');
    return data.MRData;
  } catch (error) {
    console.error('  [fetchFromErgast] Error:', error.message);
    throw error;
  }
}
